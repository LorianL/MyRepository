...
def get_data_file(filename):
    def normalise_minmax(df, key_name):
    ...
    df = normalise_minmax(df, "ROC")
    
    df = normalise_minmax(df, "volume")
...
class NeuralNet(nn.Module):
    def __init__(self):
            ...
            self.mainEncoder = Encoder()
            self.volumeEncoder = Encoder()
    ...
    def forward(self, x):
        N = x.shape[0]
        main_enc = self.mainEncoder(x[:,:,0])
        vol_enc = self.volumeEncoder(x[:,:,1])
        
        x = self.decoder(vol_enc, main_enc, vol_enc)
        x = x + main_enc
