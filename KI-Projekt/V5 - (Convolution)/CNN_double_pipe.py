...
class ConvNet_V2(nn.Module):
    def __init__(self):
        ...
        self.conv3 = nn.Conv2d(1, 10, 5)
        self.conv4 = nn.Conv2d(10, 20, 5)
        
        
        self.fc1 = nn.Linear(1152, 800)
        self.fc2 = nn.Linear(800, 600)
        self.fc3 = nn.Linear(600,300)
        self.fc4 = nn.Linear(300,80)
    ...
    def forward(self,x):
        ...
        conv_out_1 = torch.prod(torch.tensor(out_1.shape[1:])).item()
        out_1 = out_1.view(-1, conv_out_1)
        ...
        out_2 = F.relu(out_2)
        #Berechne den Output des ConvLayers
        conv_out_2 = torch.prod(torch.tensor(out_2.shape[1:])).item()
        out_2 = out_2.view(-1, conv_out_2)
        
        #print(conv_out_1 + conv_out_2)
        #Flatten the Tensor
...
lr_scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=100, verbose=True)
...
def train(epoch):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if (i+1) % 1000 == 0:
            print(f"Epoch[{epoch+1}/{EPOCHS}], Step [{i+1}/{n_total_steps}], Loss: {total_loss / (i+1):.6f}")
...
def valid(epoch):
    with torch.no_grad():
        ...
        avg_loss = total_loss / n_total_steps
        print(f"\n\nAccuracy in Epoch {epoch+1}: {acc:.4f}%     Loss: {avg_loss:.6f}\n\n")
        return acc, avg_loss
