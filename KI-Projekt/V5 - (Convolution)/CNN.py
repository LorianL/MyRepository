...
def performance_best(startcapital=50, threshold = 0.52):
    n_total_steps = len(valid_loader)
            ...
            for images, target in valid_loader:
                ...
                for i,out in enumerate(outputs):
                    if out >= threshold:
                        ...
                        print(out_diff)
                        n_samples += 1
                        before_trade = mul_profit
                        mul_profit = mul_profit*(1+out_diff)
                        sum_profit += out_diff
                        if out_diff >= 0:
                            print(f"Winning Trade: {float(out_diff)}% Profit and gained {mul_profit-before_trade}$")
                            count_True += 1
                        else:
                            print(f"Winning Trade: {float(out_diff)}% Loss and lossed {mul_profit-before_trade}$")
                            count_False += 1
