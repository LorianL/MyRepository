...
ENCODING_LENGTH = 150
TOKENIZING_LENGTH = 40 #Muss durch 2 Teilbar sein und ein Vielfaches von 5 sein
...
def categorize(df, name:str):
    ...
    df['ROC'] = pd.cut(df['ROC'], bins=bin_list, labels=labels, include_lowest=True)
...
class Encoder(nn.Module):
    def __init__(self, encoding_length, embed_size, num_layers, heads, device, forward_expansion, dropout, max_length):
        ...
        self.number_embedding_roc = nn.Embedding(encoding_length, int(embed_size/2))
        self.number_embedding_close = nn.Embedding(encoding_length, int(embed_size/2))
    ...
    def forward(self, x):
        ...
        close_emb = self.number_embedding_close(x[:,:,0])
        roc_emb = self.number_embedding_roc(x[:,:,1])
        
        num_emb = torch.cat((close_emb, roc_emb), dim=2).to(self.device)

        x = num_emb + self.position_embedding(positions)
