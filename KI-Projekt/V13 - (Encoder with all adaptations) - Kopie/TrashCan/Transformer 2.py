...
ENCODING_LENGTH = 100
TOKENIZING_LENGTH = 24

###------------------Additional Helper Functions------------------------------

def create_bin(minVal, maxVal):
    amount = ENCODING_LENGTH
    size = (abs(minVal)+abs(maxVal))/amount
    startvalue = minVal
    bin_list  = [startvalue]
...
def get_data_file(filename):
    ...
    df['ROC'] = 1000 * (df['ROC'] - min_value) / (max_value - min_value)
    
    min_value = df['ROC'].min() # Updaten des Minvalues
    max_value = df['ROC'].max() #Updaten des Maxvalues
    
    #Werte Kategoriesiern
    
    bins = create_bin(min_value, max_value)
    print(bins)
...
learning_rate = 0.00001 #0.00005 bei Batch 64
l2_lambda = 0.000001
#----------------------------Transformer------------------------------------------------------
