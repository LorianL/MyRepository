...
def categorize_per_Neuron(df, name:str):
...
def categorize_per_size(df, name:str):
    amount = ENCODING_LENGTH
    print(df.isna().sum())
    #Normalisiere auf werte zwischen 0 und 1
    df = min_max_normalize(df, name)

    #Erstellung der Bins nach size
    
    size = 1/amount
    startvalue = 0
    bin_list  = [0]
    for i in range(0,amount):
        startvalue = (startvalue + size)
...
def get_data_file(filename):
    ...
    #Rate of Change und RSI erstellen 
    
    df["ROC1"] = df["close"].pct_change()
    df["ROC2"] = df["close"].pct_change()
    #df = add_rsi(df)
    ...
    df = categorize_per_size(df, "ROC1")
    df = categorize_per_Neuron(df, "ROC2")
    #df = categorize_per_size(df, "rsi_14")
    
    #Umsortierung der Spalten
    #new_order = ["rsi_14", "ROC","close", "target"]
    #df = df[new_order]
...
learning_rate = 0.001 #0.001 bei Batch 512
l2_lambda = 0.000001 #0.000001 bei Batch 512
#----------------------------Transformer------------------------------------------------------
...
class Encoder(nn.Module):
    def __init__(self, encoding_length, embed_size, num_layers, heads, device, forward_expansion, dropout, max_length):
        ...
        self.close_embedding = nn.Embedding(encoding_length, int(embed_size/2))
        self.roc_embedding = nn.Embedding(encoding_length, int(embed_size/2))
        ...
        self.dropout = nn.Dropout(dropout)
        self.fcConnect = nn.Linear(embed_size,embed_size, bias=False)
        self.fcOut = nn.Linear(embed_size*SEQ_LEN,1)
    
    def forward(self, x):
        N, seq_length = BATCH_SIZE, SEQ_LEN
        positions = torch.arange(0,seq_length).expand(N,seq_length).to(self.device)
        
        #num1 = num_emb[:,:,:20]
        #num2 = num_emb[:,:,20:]
        
        close_out = self.close_embedding(x[:,:,0])
        roc_out = self.roc_embedding(x[:,:,0])
        pos_emb = self.position_embedding(positions)
        

        
        num_emb = torch.cat((close_out, roc_out), dim=2)
        #num_emb = self.fcConnect(num_emb)
...
optimizer = torch.optim.AdamW(model.parameters(), lr=learning_rate, weight_decay=l2_lambda) #, weight_decay=l2_lambda
