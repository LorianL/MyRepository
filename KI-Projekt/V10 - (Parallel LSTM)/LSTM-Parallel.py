...
embedding_dropout_prob = 0.02
lstm_dropout_prob = 0
dropout_prob = 0.15

learning_rate = 0.0001 #0.00005 bei Batch 64
l2_lambda = 0.000004#0.000001
leaky_slope = 0.01
WEIGHT_CLIP_VALUE = 5
GRADIENT_CLIP_VALUE = 3 #2
...
class LSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size, dropout_prob):
        ...
        self.h0_close = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(device)
        self.c0_close = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(device)
        
        self.h0_volume = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(device)
        self.c0_volume = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(device)
        
        #Layers
        self.emb_close = Embedding(n_embeddings, dim)
        self.emb_volume = Embedding(n_embeddings, dim)
        
        self.lstm_close = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True, dropout=lstm_dropout_prob) #
        self.lstm_volume = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True, dropout=lstm_dropout_prob) 
        
        
        self.fc1 = nn.Linear(2*hidden_size,int(hidden_size*expansion_factor))
    ...
    def forward(self,x):
        ...
        out_close = self.emb_close(x[:,:,0])
        out_volume = self.emb_volume(x[:,:,1])

        # Führe die Vorwärtsberechnung des RNN durch
        out_close, (h_n, c_n) = self.lstm_close(out_close, (self.h0_close, self.c0_close))
        out_close = out_close[:, -1, :]
        
        out_volume, (h_n, c_n) = self.lstm_volume(out_volume, (self.h0_volume, self.c0_volume))
        out_volume = out_volume[:, -1, :]

        x = torch.cat((out_close, out_volume), dim=1)
